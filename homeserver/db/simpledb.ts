...
export function localMapSet(state: SimpleDB, id: RawVersion, mapId: RawVersion, key: string, val: CreateValue): Operation {
...
export function setAtPath(db: SimpleDB, id: RawVersion, path: (string | RawVersion)[], val: CreateValue): Operation {
  ...
  if (key == null) {
    ...
    return localMapSet(db, id, crdtId, key, val)
  }
}

export function crdtAtPath(db: SimpleDB, path: (string | RawVersion)[]): RawVersion {
  // Written for Sets. Not sure if this is useful for other types.
  if (path.length === 0) throw Error('Invalid path')
  let [crdtId, key] = containerAtPath(db, path)
  return unwrapCRDT(db, crdtId, key) ?? errExpr('Could not unwrap')
}
